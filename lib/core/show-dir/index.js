...
const bootstrap = fs.readFileSync(path.join(__dirname,'./bootstrap.min.css'),'utf8');
      ...
      fs.readdir(dir, (readErr, _files) => {
        ...
        function render(dirs, renderFiles, lolwuts) {
          ...
          const writeRow = (file) => {
            ...
            html +=
              `<td class="display-name"><a href="${href}" class="d-block">${displayName}</a></td>` +
              `<td class="file-size"><code>${sizeToString(file[1], humanReadable, si)}</code></td>` +
              `<td class="last-modified"><small>${lastModifiedToString(file[1])}</small></td>` +
              '</tr>\n';
          };
